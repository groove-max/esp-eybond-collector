...
TEST(core_vdtu_capabilities_string) {
  ...
  CHECK_STR(build_vdtu_capabilities(profile, config),
            "esp-collector,0.1.0;features=local_only,no_cloud,wifi_params,endpoint_write;uart=2400,8,1,NONE;"
  ...
  CHECK_STR(build_vdtu_capabilities(profile, config),
            "esp-collector,0.1.0;features=local_only,no_cloud,wifi_params,endpoint_write;uart=9600,8,1,NONE;"
...
TEST(core_vdtu_served_over_tcp) {
  ...
  CHECK(calls.size() == 1);
  CHECK_STR(std::string(calls[0].data.begin(), calls[0].data.end()),
            "AT+VDTU:esp-collector,0.1.0;features=local_only,no_cloud,wifi_params,endpoint_write;uart=2400,8,1,NONE;"
            "spacing_ms=850;queue=4\r\n");
}
