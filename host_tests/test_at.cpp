...
TEST(at_vdtu_capability_probe) {
  ...
  CHECK_STR(reply_for("AT+VDTU?\r\n"),
            "AT+VDTU:esp-collector,0.1.4;features=local_only,no_cloud,wifi_params,endpoint_write,reboot;"
            "uart=2400,8,1,NONE\r\n");

  CollectorProfile profile = test_profile();
  profile.vdtu = "esp-collector,0.1.4;features=local_only,no_cloud,wifi_params,endpoint_write,reboot;uart=2400,8,1,NONE;spacing_ms=850;queue=4";
  AtCommand command;
